...
define("modal", ["marionette"], (function (global) {
    return function () {
        var ret, fn;
        return ret || global.Backbone.Modal;
    };
}(this)));
...
(function() {
  ...
  define('lib/dom_app_event_trigger',['jquery'], function($) {
...
(function() {
  define('blog',['marionette', 'lib/dom_app_event_trigger'], function(Marionette, DomAppEventTrigger) {
    var Blog;
    Blog = (function() {
...
define('mdown!articles/useful-backbone-extensions.md',[],function () { return '<p><time>March 26, 2013</time> </p>\n\n<h2>Useful Backbone extensions</h2>\n\n<p>Backbone 1.0.0 was recently released. In this article we\'ll talk about why Backbone has become such a great platform. You probably already know Backbone and know why it\'s awesome, but the extensions make it even more spectacular. <a href="https://github.com/davidvanleeuwen/blog">See how this code works</a>.</p>\n\n<h3>The awesomesauce</h3>\n\n<p>Backbone is a library and you can use it however you want. When you\'re working on more complex apps, you\'ll run into certain problems or want to give a user a better experience. </p>\n\n<p>Last week I talked about Marionette, which is an extensions that adds functionality so you can write your app faster/better. The past couple of months I\'ve came across several extensions, which I think can make your life so much easier as a Backbone developer. That\'s why I\'ve created a top 5.</p>\n\n<p>In this article we\'ll use some of the extensions to improve this blog. We\'ll create chapters for future articles and add view bindings, so we\'ll be ready for all upcoming articles.</p>\n\n<h3>5. Paginator</h3>\n\n<p>We\'re not using the <a href="https://github.com/addyosmani/backbone.paginator">Backbone.Paginator</a> for this blog, but it\'s really useful when creating an app that is in need of pagination. There are several paginator extensions out there, but this will get the job done in a simple matter.</p>\n\n<p>It has two components, one meant for clientside pagination and one meant for pagination hooked up to your API. It extends from a <code>Backbone.Collection</code> and you can add your own options for serverside handling and UI handling. But you\'ll have to implement your own UI for pagination, which makes it more flexible - but requires some more coding on your end.</p>\n\n<h3>4. Validation</h3>\n\n<p>For most applications you\'ll need to do some validation. Not for this blog, but whenever you want to validate your form and do some UI work with it, you should definitely look at <a href="https://github.com/thedersen/backbone.validation">Backbone.Validation</a>. It\'s a simple and effective extension.</p>\n\n<p>This extension is especially useful in combination with other extensions in this list.</p>\n\n<p><img src="/images/shipment_modal_validation.png" alt="Shipment Modal - Validation example" /></p>\n\n<p>This happens when you forgot to enter a project name in <a href="http://shipmentapp.com">Shipment</a>.</p>\n\n<h3>3. Relational</h3>\n\n<p>Having nested data is pretty common in larger projects. If you have an array in your model or even a nested user object in a model, you\'ll want to use <a href="http://backbonerelational.org/">Backbone.Relational</a>.</p>\n\n<p>There are two relation types, a <code>HasOne</code> and a <code>HasMany</code>, so you can use a nested model or a nested collection as a relation.</p>\n\n<p>For future articles we\'ll want to group articles to get things organized. Let\'s create a chapter model with a <code>HasMany</code> relation so it can contain an articles collection.</p>\n\n<p><code data-gist="https://gist.github.com/5241820.json"></code></p>\n\n<p>It extends from a <code>Backbone.RelationalModel</code>. Within the <code>relations</code> array you can define all relations, but for our blog we\'ll only need the collection with articles for every chapter. I\'ve also changed the fixtures to make this happen.</p>\n\n<p>We\'ll need to change some other things too. Our article event needs to change, because it\'ll have to search through every chapter to find the article you are looking for:</p>\n\n<p><code data-gist="https://gist.github.com/5241905.json"></code></p>\n\n<p>Instead of searching through a simple collection, we\'ll need to create a method to search through every chapter. So on the top of our article event, will call <code>chapters.findArticleBySlug(slug)</code> and if it isn\'t found it will just grab the last one.</p>\n\n<p><code data-gist="https://gist.github.com/5241925.json"></code></p>\n\n<p>This returns the chapter that includes an article that has a slug that is similar as the parameter.</p>\n\n<h3>2. Stickit</h3>\n\n<p>Instead of manually setting your model attributes for every view and render it again for every change, you\'ll be probably better of using a view-binding extension. Luckily there are several out there.</p>\n\n<p><a href="https://github.com/theironcook/Backbone.ModelBinder">Backbone.Modelbinder</a> is to most basic one, which is pretty straightforward and simple to use. But if you want to do some more custom things it\'s becomes pretty cumbersome.</p>\n\n<p>If you prefer the <a href="http://angularjs.org/">AngularJS</a> approach, you can use <a href="http://rivetsjs.com/">RivetJS</a>. It works by declaring your bindings in the templates itself.</p>\n\n<p>Although I\'m used to Modelbinder, I think I\'m going to use Stickit from now on. As Modelbinder, you can declare your bindings in the <code>Backbone.View</code> and not in your templates. Your templates should stay simple and clean. Take a look at the bindings for our <a href="https://github.com/davidvanleeuwen/blog/blob/master/src/scripts/views/navigation_item.coffee">NavigationItem</a>:</p>\n\n<p><code data-gist="https://gist.github.com/5241490.json"></code></p>\n\n<p>As <a href="http://nytimes.github.com/backbone.stickit/">the site</a> says: you only need to define <code>view.bindings</code> and call <code>@stickit()</code>. The value in the hash is the selector, so you can grab the element you want to use from your template. The key is the attribute from your model, which is <code>@model</code> by default.</p>\n\n<p>You can do all kinds of awesome things with Stickit. If you look at the anchor, you\'ll see that we\'re targeting two attributes. The <code>href</code> is used to set a slug but change it\'s output value. We\'re also setting the <code>class</code> attribute to active when it\'s true.</p>\n\n<h3>1. Memento</h3>\n\n<p>You might find that the previous extensions are pretty straightforward and also pretty common things that you can use for every app. You\'ll also find these things in other libraries/frameworks. With Backbone though, you\'ll have a choice and you can pick your own approach. You\'re app stays lean, Backbone only handles the essentials and you can find or create extensions that makes your app more complete.</p>\n\n<p>So the #1 at this point is certainly <a href="https://github.com/derickbailey/backbone.memento">Backbone.Memento</a>. It\'s really useful for particular scenerio\'s and makes the difference for your app.</p>\n\n<p>Memento is an extension that remembers the state of your models and you can create a snapshot of every moment and go back to it whenever you want. It\'s something that\'s unique and gives your app a real native feel.</p>\n\n<p>It\'s not in this blog, but one scenario is: in Shipment you can open up a modal with your settings. You can edit your profile and it saves the model when you press done. You can also switch between views, but it\'ll only set the attributes if you do - so when you switch back to that view, it\'ll still have those changed values. But whenever you cancel it by pressing esc, it calls <code>memento.restore()</code>. Now it restores the model to the original state. Just the way you expect it to work.</p>\n\n<h3>Next chapter</h3>\n\n<p>This chapter has covered the start of this blog using several techniques using JavaScript. It\'s still an experiment, but I\'ve received some great feedback and we\'ll be improving it on a weekly basis.</p>\n\n<p>The next chapter will cover a new project, which will be an extension to Backbone. It will start small, but with every article we\'ll figure out how to create an open source project.</p>\n\n<p>Stay tuned and if you have any feedback or suggestions. Tweet to <a href="https://twitter.com/davidvanleeuwen">@davidvanleeuwen</a></p>';});

define('mdown!articles/getting-started.md',[],function () { return '<p><time>April 4, 2013</time> </p>\n\n<h2>Getting started</h2>\n\n<p>After building this blog, I thought it would be great to open source a project that I\'m working on. This is an introduction in creating our own Backbone extension and starts with Jasmine.</p>\n\n<iframe src="http://player.vimeo.com/video/63251093" width="700" height="393" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>\n\n<h3>This chapter: Backbone.Modal</h3>\n\n<p>In a nutshell: modals in web apps are really useful. They\'re popular because it gives your app a native feel and it can improve the user experience, because you don\'t leave the page while still focussing on something else.</p>\n\n<p>So what are we up to? Shipment uses an extension for Backbone that makes it easy to create all kinds of modals. Creating a modal with multiple tabs, that switches from one (Backbone) view to another. It handles all the events that you would expect from a modal, for example: press esc or click outside the modal to close and enter to submit.</p>\n\n<p>The next series of posts will cover how we\'ll build a Backbone extension: <strong><a href="https://github.com/davidvanleeuwen/backbone.modal">Backbone.Modal</a></strong>. I\'ve created the repo with Grunt (love it) and added Jasmine to it, <a href="https://github.com/davidvanleeuwen/backbone.modal">check out the current code</a>.</p>\n\n<h3>What\'s Jasmine?</h3>\n\n<p>As it won\'t be just a simple dialog modal, we want to make sure everyhing works correctly. And instead of just building it, it would be smarter to write down it\'s functionality before we start writing any code.</p>\n\n<p><a href="http://pivotal.github.com/jasmine/">Jasmine</a> is a testing framework for your Javascript, built by the guys at Pivotal Labs. They call it behavior-driven development, because you write your tests with "What should happen when x happens?". </p>\n\n<p>It\'s a different kind of workflow, but is easy to get used to. It starts with "what should this do?", describe it in a test like: "it should do..." and the test fails. This is because we haven\'t written any code yet (besides the test itself). So you\'ll start fixing it by writing the code. Repeat this and you\'ll have fully functional code that is tested.</p>\n\n<h3>The spec</h3>\n\n<p>Let\'s write down the specification for Backbone.Modal. This is just my first thought and is clearly not finished yet. But let\'s have a look:</p>\n\n<p><code data-gist="https://gist.github.com/5293941.json"></code></p>\n\n<p>Easy peasy. <code>Describe</code> the tests, what <code>it</code> should do and there you go. The only thing you\'re probably wondering is the <code>#</code> in there. This is just a convention to use when you\'re describing a method from your code.</p>\n\n<p>When you run the code, by using <code>grunt watch</code> from your terminal, it\'ll open up the spec runner. This is generated by Jasmine that shows which tests are passing and which are failing.</p>\n\n<p><img src="/images/jasmine_tests.png" alt="Backbone Modal - Jasmine" /></p>\n\n<p>As you can see all tests are passing right now. That\'s because every test is a function. CoffeeScript will by default add a return at the end of every function. Let\'s write the code that actually evaluates the test.</p>\n\n<h3>Getting things done</h3>\n\n<p>So one by one we\'ll write down what we expect every piece of code (spec) to do. Every spec returns false or true to pass the test. By using Jasmine\'s convience method <code>expect()</code>, it makes it easy to test every result you want.</p>\n\n<p><code data-gist="https://gist.github.com/5306080.json"></code></p>\n\n<p>The methods <code>beforeEach</code> and <code>afterEach</code> are methods that Jasmine runs before and after each spec. So we\'ll define a <code>modal</code> class and create an instance. Because that\'s what we\'re testing here...</p>\n\n<p>On the <a href="http://pivotal.github.com/jasmine/">Jasmine site</a> you find all expectations, like <code>toBeDefined</code> and <code>toThrow</code> which we\'re using in this example.</p>\n\n<h3>Now what?</h3>\n\n<p>With every spec I started writing the <a href="https://github.com/davidvanleeuwen/backbone.modal/blob/master/src/backbone.modal.coffee">code for this</a>:</p>\n\n<p><code data-gist="https://gist.github.com/5309171.json"></code></p>\n\n<p>You\'ll see that I\'ve created the methods to pass the above tests. If you\'ve read the tests what <code>it</code> should do, I don\'t need to explain what\'s happening in this code. The extension isn\'t really useful yet, but the next couple of weeks this extension will be more complete and useful for your projects too.</p>\n\n<p>My plan for this article was to write all Jasmine tests and explain every bit of it. But I came to the conclusion that writing every test, will force me to write the whole extension. This article scratches the surface of Jasmine and is just the beginning.</p>\n\n<h3>Next up</h3>\n\n<p>We\'ll be writing more core functionality in the next article. As you see in the Shipment modals, there is also a lot of styling happening. My colleague and friend <a href="https://twitter.com/fverloop">Floris</a> will help me with setting this up.  Please let me know what you think of Jasmine, Backbone.Modal or my blog in general by tweeting to <a href="https://twitter.com/davidvanleeuwen">@davidvanleeuwen</a>.</p>';});

define('mdown!articles/creating-the-modal.md',[],function () { return '<p><time>May 9, 2013</time> </p>\n\n<h2>Creating the modal</h2>\n\n<p>Woa! I didn\'t knew using Jasmine would help me unbore writing code. As frontend developer - not sure what that means though - I\'ve always had the need to check the results on screen in a browser. After writing some lines of code, I always wanted to see things move or having something to click on. With Jasmine however I don\'t have that urge anymore.</p>\n\n<p>This article is the second part of writing a Backbone extension. Why still bother you might ask. Shouldn\'t you be using Angular by now? I always believed that Backbone\'s power is it\'s community and the loads of <a href="/articles/useful-backbone-extensions">awesome extensions</a> it has. Plus, we\'ll let this extension work with <a href="/articles/backbone-marionette">Marionette</a> as well, which is a powerful framework I recommend using instead of Angular or any other framework. Backbone and Marionette are really robust and amazingly flexible, which is a perfect combination and that\'s what made me fell in love with it.</p>\n\n<h3>Here we go</h3>\n\n<p>Last time we discussed the functionality of the modal and wrote the spec. Instead of writing the actual code, I wanted to describe first what it should do. But everytime you write a test, you want it to pass. Let\'s take a look at what we\'ve added so far:</p>\n\n<p><code data-gist="https://gist.github.com/5478075.json"></code></p>\n\n<p>We\'re using Jasmine\'s Spies here. Those things are pretty awesome. By using <code>spyOn</code> you can spy on an object and check which methods are being called. After creating an instance of the <code>modal</code>, you can spy on methods you call directly or internally. So when calling <code>view.render()</code> it should call <code>triggerView</code> internally. This test checks if this is true. So whenever we change the render method, this test will help us know if it\'s stil doing the thing it should be doing.</p>\n\n<h3>Some other Jasmine goodness</h3>\n\n<p>With Jasmine you can test every outcome and every method, but you can also define your own custom matchers. By calling <code>addMatchers</code> before each test you can create a expectation method, that you can re-use within the suite you\'re writing.</p>\n\n<p>When creating larger applications, you\'re probably also have methods that run asynchronous. Jasmine provides two methods for this. By calling <code>waitFor</code> with a condition, it\'ll wait until it returns true. By writing your expectation within a <code>runs</code> block, it will run each function on after another.</p>\n\n<p>These things are pretty staightforward and really useful, so if you\'d like to know more: <a href="http://pivotal.github.io/jasmine/">check out the docs</a>.</p>\n\n<h3>What do we have now</h3>\n\n<p>After running through the spec, I\'ve wrote the <a href="https://github.com/davidvanleeuwen/backbone.modal/blob/master/src/backbone.modal.coffee">initial code</a> for the modal. There\'s no real usable result yet, because <a href="https://twitter.com/fverloop">@fverloop</a> is working on the styling, to create beautiful modals like the Shipment modals.</p>\n\n<p>This will be the code you\'ll need to setup a modal with two tabs for example:</p>\n\n<p><code data-gist="https://gist.github.com/5478570.json"></code></p>\n\n<p>It\'s a subclass of a Backbone.View, which you can pass a <code>template</code>. But to have a modal with several tabs, you will want to define multiple <code>views</code>. So if you add a view and pass it with a selector, it\'ll do the rest of the work. A template is optional, because you can just only use the views here. But when using a template, you\'ll have to define a <code>viewContainer</code>, which it uses to render the views in.</p>\n\n<p>In the template you\'ll add the markup for the tabs, a close button and a submit button for example. These things will stay in the modal, whenever you switch between tabs. Everytime a selector is triggered, it will grab the view and render it in the viewContainer.</p>\n\n<h3>Using Backbone.Modal with Marionette</h3>\n\n<p>In the previous chapter we ran through Backbone.Marionette, an amazing framework to make your application even more awesome. You don\'t need to use every part of it, but can choose whatever you\'d like - just like how Backbone does it. So let use <code>Marionette.Layout</code> to show a modal. It would be great to support stacked modals too, like Shipment does. By creating a custom <code>Marionette.Region</code>, we could stack a modal view everytime you call <code>show</code>. And we should add a convience method to close all the modals. Here you go:</p>\n\n<p><code data-gist="https://gist.github.com/5550062.json"></code></p>\n\n<p>For now it just adds each modal view you add to an array and everytime you call <code>close</code>, it closes the last modal in the stack. Pretty simple and straightforward, but the way to do stack modals with Marionette.</p>\n\n<p>And this is how the example looks like now:</p>\n\n<p><code data-gist="https://gist.github.com/5550096.json"></code></p>\n\n<p>You can create a layout class, add a region with <code>regionType: Backbone.Marionette.Modals</code> and create a modal and add it to that region and do that again. Of course you shouldn\'t be adding the same modal view, but this is just to get the idea. Now you\'ve got a stacked modal.</p>\n\n<h3>Next time</h3>\n\n<p>We\'re about to add the styling and do the animating stuff. When that\'s done it\'s really going to look amazing and work pretty solid. Next time we\'re going to talk about releasing Backbone.Modal in the wild and let you play with it.</p>\n\n<p>Let me know what you think of this article: <a href="http://twitter.com/davidvanleeuwen">@davidvanleeuwen</a></p>';});

define('mdown!articles/backbone.modal-release.md',[],function () { return '<p><time>August 31, 2013</time> </p>\n\n<h2>Backbone.Modal release</h2>\n\n<p>Yay, it\'s finally done. We just launched the <a href="http://awkward.github.io/backbone.modal/">first version of Backbone.Modal</a>. It took a while, but it\'s pretty amazing. We made sure it\'s flexible enough by testing and implementing it in several of our projects. And I can say, it works like a charm.</p>\n\n<p>In this article we\'re going to talk about how you can actually use Backbone.Modal. In the previous articles we discussed how we could build such a plugin. So let\'s put that into practice and add a modal to this blog.</p>\n\n<p><img src="/images/modals.png" alt="Backbone.Modal" width="100%" /></p>\n\n<p>Do check out the <a href="http://awkward.github.io/backbone.modal/">docs</a> we\'ve created for you. There are some examples on there and you can try them out on the site itself.</p>\n\n<h3>Your first modal</h3>\n\n<p>Thanks to <a href="https://twitter.com/fverloop">Floris</a>, we added some cool animations and styles. We don\'t need to go through all the trouble to add a default style and start playing with it. To get everything up and running, just add <code>backbone.modal-min.js</code>, <code>backbone.modal.css</code> and <code>backbone.modal.theme.css</code> to your site.</p>\n\n<p>Make sure you have all the required dependencies on your site: jQuery, underscore.js and backbone.js ofcourse.</p>\n\n<p>If you want to use the default style, like the backdrop, the modal itself, the button and title, you\'ll need to include some elements that start with <code>bbm-</code>. This is in HAML:</p>\n\n<p><code data-gist="https://gist.github.com/6394432.json"></code></p>\n\n<p>Then we create the Backbone.Modal view, which inherits from a plain Backbone.View. This means you can do whatever you would do normally with your view, but this adds some sugar, to create a simple modal:</p>\n\n<p><code data-gist="https://gist.github.com/6394442.json"></code></p>\n\n<p>Yep, in CoffeeScript and uses RequireJS too, because that\'s what we\'re using for this blog - but you can use whatever you\'d prefer. So far, so good. Nothing fancy really, just a simple modal that pops up: <a href="#" data-event="modal">show me the result</a>.</p>\n\n<h3>The cool things</h3>\n\n<p>In the <a href="/articles/getting-started">first article</a> of this chapter I added a clip that show a few example modals from: Stripe, Shipment and Karma. If you look at Stripe\'s account modal, it has 9 tabs and every view has a button that opens another small modal on top. You can do something similar with Backbone.Modal, without writing a lot of code. Check out this <a href="https://github.com/awkward/backbone.modal/blob/master/examples/2_tab_based.html">example on Github</a>.</p>\n\n<p>There are some more examples, one of this is creating a stacked modal. We have also discussed Marionette, which is an awesome framework made by <a href="https://twitter.com/derickbailey">Derick Bailey</a>. When you\'re using it too, you can create stacked modals by adding a region type in your Marionette.Layout.</p>\n\n<p>Here\'s what the layout for this blog looks like:</p>\n\n<p><code data-gist="https://gist.github.com/6394633.json"></code></p>\n\n<p>Now whenever you call <code>layout.show(modal)</code> multiple times, it will open every modal on top of the other: <a href="#" data-event="modals">let\'s try it out</a>.</p>\n\n<h3>What now?</h3>\n\n<p>I would love to hear your feedback. And absolutely let me know if you\'ve added this to any of your projects. Contact me: <a href="https://twitter.com/davidvanleeuwen">@davidvanleeuwen</a>.</p>\n\n<p>The next time we\'ll do something completely different. I think it\'s time to take this blog to the next level.</p>\n\n<p>PS: I also released a site in the meantime, which is a list of all Backbone plugins: <a href="http://backplug.io">backplug.io</a>.</p>';});

(function() {
  define('fixtures/articles',['mdown!articles/about-this-blog.md', 'mdown!articles/build-a-blog-using-grunt.md', 'mdown!articles/backbone.marionette.md', 'mdown!articles/useful-backbone-extensions.md', 'mdown!articles/getting-started.md', 'mdown!articles/creating-the-modal.md', 'mdown!articles/backbone.modal-release.md'], function() {
    return [
      {
...
(function() {
  define('templates/modal',[],function() {
    return function(context) {
      var render;
      render = function() {
        var $o;
...
(function() {
  ...
  define('views/modal',['templates/modal'], function(template) {
    var _ref;
...
(function() {
  define('events/articles',['collections/chapters', 'views/article', 'views/navigation', 'views/not_found', 'views/modal'], function(chapters) {
    var article;
    article = {};
...
(function() {
  ...
  require(['jquery', 'backbone', 'relational', 'stickit', 'modal', 'blog', 'events/articles', 'routers/main', 'layouts/index', 'lib/gists'], function($, Backbone) {
